...
import {
  type CaptionMode,
  initAutocaptionEditor,
  initBlankEditor,
  initImportEditor,
  initPreCaptionedEditor
} from '../imgly';
...
const CAPTION_MODES: Array<{
  ...
  },
  {
    ...
    description: 'Upload or create a video, then manually add or import captions.',
...
export function App({ editorConfig }: AppProps) {
  ...
  const openEditor = useCallback(
    async (mode: CaptionMode) => {
      ...
      cesdk.ui.registerComponent('ly.img.close.navigationBar', ({ builder }) => {
      ...
      switch (mode) {
        case 'autocaption':
          await initAutocaptionEditor(cesdk);
          break;
        case 'blank':
          await initBlankEditor(cesdk);
          break;
        case 'import':
          await initImportEditor(cesdk);
          break;
        case 'pre-captioned':
          await initPreCaptionedEditor(cesdk);
          break;
      }
    },
